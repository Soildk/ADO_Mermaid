...
> ⚠️ **UPSTREAM FILE — NOT AZURE DEVOPS CORRECT**
>
> This guide comes from the upstream `design-doc-mermaid` skill and targets GitHub /
> modern Mermaid. Its examples use ```` ```mermaid ```` fences and may reference diagram
> types Azure DevOps cannot render (`erDiagram`, `C4Context`, `mindmap`, `gitGraph`).
>
> **Use it for layout and modelling patterns only.** For anything about fences, diagram
> types, or syntax, `../../SKILL.md` and `../ado-wiki-reference.md` override this file.
> Convert every example to the `::: mermaid` fence before use.
